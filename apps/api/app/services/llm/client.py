...
class LiteLLMClient:
    ...
    Supports:
    - complete(): raw text completion (backward-compatible)
    ...
    def __init__(
        ...
        # Tell LiteLLM to silently drop params unsupported by the target
        # model (e.g. temperature for gpt-5 which only accepts temperature=1).
    ...
    def _configure_litellm() -> None:
        ...
        try:
            ...
            # Suppress verbose DEBUG logging (full request/response JSON dumps)
    ...
    async def complete(
        ...
        """Raw text completion (backward-compatible with OpenAIClient).

        Used when you don't need structured output (e.g., field identification).
        """
