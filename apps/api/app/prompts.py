"""LLM prompt templates and user prompt builder for fill/ask pipeline."""
...
MAP_SYSTEM_PROMPT = """You are a Japanese PDF form field identification expert.
...
def _direction_score(field_bbox: tuple[float, float, float, float], label_bbox: tuple[float, float, float, float]) -> tuple[float, str]:
    ...
    # Vertical alignment bonus: same row → ×0.7
    avg_height = (fh + lh) / 2
...
def build_map_prompt(
    ...
    # Few-shot section from confirmed annotations
...
def build_fill_prompt(
    ...
    """Build the user-turn prompt from session context, field label maps, and annotations."""
    form = ctx.form
    if form is None:
        return ""

    # Build field list enriched with label/semantic_key from Map results
    map_by_field = {m.field_id: m for m in field_label_maps}
    field_list = []
    for f in form.fields:
        entry: dict = {"field_id": f.id, "name": f.name, "type": f.field_type.value}
        flm = map_by_field.get(f.id)
        if flm:
            if flm.label_text:
                entry["label"] = flm.label_text
            if flm.semantic_key:
                entry["semantic_key"] = flm.semantic_key
        field_list.append(entry)

    annotation_list = [
        {"label": a.label_text, "field_id": a.field_id, "field_name": a.field_name}
        for a in annotations
    ]

    parts = [
        f"Form fields (with identified labels where available):\n{json.dumps(field_list, ensure_ascii=False)}",
        f"User information:\n{json.dumps(ctx.user_info.data, ensure_ascii=False)}",
    ]
    if annotation_list:
        parts.append(f"Manual annotations (confirmed label-field pairs):\n{json.dumps(annotation_list, ensure_ascii=False)}")
    deterministic_rules = [i.rule_text for i in ctx.rules.items if i.type != RuleType.CONDITIONAL]
    if deterministic_rules:
        parts.append(f"Rules:\n{json.dumps(deterministic_rules, ensure_ascii=False)}")
    if user_message:
...
def build_understand_prompt(fields: list[FormField], text_blocks: list[TextBlock]) -> str:
    """Build prompt for document rule extraction."""
    field_list = [
